...
module.exports = function() {
  ...
  (async function(){
    ...
    // Categorize videos
    let checkVideos = {} // videos to be checked for live status
    let upcomingVideos = {} // upcoming videos only
    ...
    for (videoKey in upcomingVideos) {
      let videoInfo = upcomingVideos[videoKey]
      let scheduleMoment = moment(videoInfo.liveSchedule)
    ...
    if (Object.keys(checkVideos).length) {
      ...
      let videoKeys = Object.values(checkVideos).map(v => v.ytVideoId).slice(0, 50).join(',')
      ...
      for (videoInfo of videoInfos) {
        ...
        let videoKey = 'yt:' + videoId
        let oldInfo = checkVideos[videoKey]
        // Video livestream status
        if (videoInfo.liveStreamingDetails) {
          let liveSchedule = videoInfo.liveStreamingDetails.scheduledStartTime || null
          let liveStart = videoInfo.liveStreamingDetails.actualStartTime || null
          let liveEnd  = videoInfo.liveStreamingDetails.actualEndTime || null
          let liveViewers  = videoInfo.liveStreamingDetails.concurrentViewers || null
          // console.log('videosLive() liveStreamingDetails', videoId, oldInfo.status, '|', liveSchedule, '|', liveStart, '|', liveEnd, '|', liveViewers)
          let scheduleMoment = moment(liveSchedule)
          let startMoment = moment(liveStart)
          let endMoment = moment(liveEnd)
    ...
    for (videoKey in updateVideos) {
      let updateData = updateVideos[videoKey]
